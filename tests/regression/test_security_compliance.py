...
def test_sha256_audit_chain_tamper_detection():
    ...
    h = hashlib.sha256()
    h.update(str(events[2]["entity_id"]).encode("utf-8"))
    h.update(str(events[2]["action"]).encode("utf-8"))
    h.update(str(events[2]["payload"]).encode("utf-8"))
    h.update(str(events[2]["timestamp"]).encode("utf-8"))
    prev_h = events[2]["prev_hash"]
    if prev_h:
        h.update(prev_h.encode("utf-8"))
