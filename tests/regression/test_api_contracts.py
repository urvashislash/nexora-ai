...
EXPECTED_API_CONTRACT: dict[str, dict[str, Any]] = {
...
def test_api_contracts_unchanged():
    ...
    for endpoint, contract in EXPECTED_API_CONTRACT.items():
        assert endpoint in actual_api_contract, (
            f"Endpoint {endpoint} is missing from API."
        )

        actual_contract = actual_api_contract[endpoint]

        for field in contract["required_fields"]:
            assert field in actual_contract["required_fields"], (
                f"Required field {field} missing in {endpoint}"
            )

        for field in contract["optional_fields"]:
            assert field in actual_contract["optional_fields"], (
                f"Optional field {field} missing in {endpoint}"
            )

        for resp_field, resp_type in contract["response"].items():
            assert resp_field in actual_contract["response"], (
                f"Response field {resp_field} missing in {endpoint}"
            )
            assert actual_contract["response"][resp_field] == resp_type, (
                f"Response type mismatch for {resp_field} in {endpoint}"
            )
