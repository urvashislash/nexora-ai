...
class AIQueueWorker:
    ...
    def _load_evidence(self, message: ProcessDocumentMessage) -> bytes | str:
        ...
        if not settings.SUPABASE_URL or not settings.SUPABASE_SERVICE_ROLE_KEY:
            raise MediaBackendUnavailableError(
                "Supabase storage credentials are required to load storage_key evidence"
            )
        ...
        try:
            ...
            if response.status_code >= 500:
                raise EvidenceAccessError(
                    f"Evidence storage returned HTTP {response.status_code}", retryable=True
                )
            if response.status_code >= 400:
                raise EvidenceAccessError(
                    f"Evidence storage returned HTTP {response.status_code}", retryable=False
                )
    ...
    def process_job(self, message: ProcessDocumentMessage) -> dict[str, Any]:
        ...
        if cached_observations is None:
            ...
            evidence_fingerprint = self.state_store.get_checkpoint(job_id, "EVIDENCE_FINGERPRINT")
        ...
        try:
            ...
            try:
                if retryable and next_attempt < settings.QUEUE_MAX_ATTEMPTS:
                    if job_id:
                        self.state_store.fail(job_id, str(exc), final=False)
                    self._publish(
                        "document.retry",
                        body,
                        correlation_id=message.correlation_id if message else getattr(properties, "correlation_id", None),
                        headers={"x-attempt": next_attempt, "x-error": str(exc)[:500]},
                    )
                else:
                    if job_id:
                        self.state_store.fail(job_id, str(exc), final=True)
                    failure = {
                        "correlation_id": message.correlation_id if message else getattr(properties, "correlation_id", None),
                        "idempotency_key": job_id,
                        "job_id": job_id,
                    ...
                    self._publish(
                        ...
                        headers={"x-idempotency-key": job_id or "invalid-message"},
                    )
                    self._publish(
                        "document.failed",
                        body,
