          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6 mt-0">
              ...
              <div className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-sans text-slate-600 block font-medium">Execution Status</span>
                    <div className="mt-1">
                      <NexoraStatusBadge status={status} />
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-sans text-slate-600 block font-medium">Reconciled Progress</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-1">
                  <span className="text-[10px] font-sans font-semibold uppercase text-slate-600">Planned Start / Finish</span>
                  ...
                  <span className="text-[10px] text-slate-600 font-sans font-medium">Baseline Schedule</span>
                </div>

                <div className="p-4 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-1">
                  <span className="text-[10px] font-sans font-semibold uppercase text-slate-600">Actualized Dates</span>
                  <div className="text-xs font-mono font-semibold text-slate-900">
                    {state?.actual_start_date || '—'} &rarr; {state?.actual_finish_date || '—'}
                  </div>
                  <span className="text-[10px] text-slate-600 font-sans font-medium">
                    Variance: <span className={state?.variance_days && state.variance_days > 0 ? 'text-rose-800 font-bold' : 'text-emerald-800 font-bold'}>
              ...
              <div className="p-5 bg-white rounded-2xl border border-slate-200/80 shadow-2xs space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-sans">
                  <div>
                    <span className="text-slate-600 block font-medium">Planned Quantity</span>
                    <span className="font-semibold text-slate-900">
                      {activity.planned_quantity ? `${activity.planned_quantity} ${activity.unit_of_measure || 'Units'}` : '—'}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-600 block font-medium">Cumulative Installed</span>
                    <span className="font-semibold text-slate-900 font-mono">
                      {state?.cumulative_quantity || 0} {activity.unit_of_measure || 'Units'}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-600 block font-medium">Equipment / Line Tag</span>
                    <span className="font-mono text-slate-900 font-medium">{activity.equipment_tag || '—'}</span>
                  </div>
                  <div>
                    <span className="text-slate-600 block font-medium">Schedule Weightage</span>
            ...
            <TabsContent value="evidence" className="space-y-4 mt-0">
              {linkedObservations.length === 0 ? (
                <div className="p-8 text-center bg-slate-50/70 rounded-2xl border border-slate-200/70 space-y-2">
                  <Cpu className="h-8 w-8 text-slate-500 mx-auto" aria-hidden="true" />
                  <p className="text-xs font-semibold text-slate-800 font-sans">No Linked Field Evidence Yet</p>
                ...
                linkedObservations.map((obs) => (
                  <div key={obs.id} className="p-4 bg-white rounded-2xl border border-slate-200/80 space-y-2.5 shadow-2xs">
                    ...
                    <p className="text-xs text-slate-800 bg-slate-50/70 p-3 rounded-xl border border-slate-200/70 font-sans leading-relaxed italic">
                      "{obs.raw_text}"
                    </p>

                    <div className="flex items-center justify-between text-[11px] text-slate-600 font-sans pt-1 font-medium">
            ...
            <TabsContent value="dependencies" className="space-y-5 mt-0">
              ...
              <div className="space-y-2">
                <span className="text-xs font-sans font-semibold uppercase tracking-wider text-slate-700 block">
                  Predecessors ({predecessors.length}) &bull; Finish-to-Start (FS)
                </span>
                {predecessors.length === 0 ? (
                  <div className="p-3 bg-slate-50/70 rounded-xl border border-slate-200/70 text-xs text-slate-600 font-sans">
                  ...
                  predecessors.map((p) => (
                    <div key={p.activity.id} className="p-3 bg-white rounded-xl border border-slate-200/80 flex items-center justify-between shadow-2xs">
                      <div>
                        ...
                        <span className="text-xs text-slate-800 font-sans">{p.activity.name}</span>
                      </div>
                  ...
                  successors.map((s) => (
                    <div key={s.activity.id} className="p-3 bg-white rounded-xl border border-slate-200/80 flex items-center justify-between shadow-2xs">
                      <div>
                        ...
                        <span className="text-xs text-slate-800 font-sans">{s.activity.name}</span>
                      </div>
            ...
            <TabsContent value="audit" className="space-y-3 mt-0">
              {linkedAudits.length === 0 ? (
                <div className="p-8 text-center bg-slate-50/70 rounded-2xl border border-slate-200/70 space-y-1">
                  <ShieldCheck className="h-7 w-7 text-slate-500 mx-auto" aria-hidden="true" />
                  <p className="text-xs font-semibold text-slate-800 font-sans">No Audit Ledger Entries</p>
                  <p className="text-xs text-slate-600 font-sans">No immutable ledger entries recorded yet for this activity.</p>
                </div>
              ) : (
                linkedAudits.map((evt) => (
                  <div key={evt.id} className="p-4 bg-white rounded-2xl border border-slate-200/80 space-y-2 shadow-2xs">
                    ...
                    <div className="font-mono text-[11px] text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-200/70 truncate">
